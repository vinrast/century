...
<?php
	...
	if (($gestor = fopen(DATA_PATH."propiedades.csv", "r")) !== FALSE) {
		...
		while (($datos = fgetcsv($gestor, 0, ",",'"')) !== FALSE) {
			...
			$import = "INSERT INTO propiedades values (
											 ...
											 '".mysql_real_escape_string($datos[0])."', 0, 
	...
	//mysql_query("UPDATE propiedades SET tipo_inmueble = convert(cast(convert(tipo_inmueble using latin1) as binary) using utf8)");

	//mysql_query("UPDATE propiedades SET comentarios = convert(cast(convert(comentarios using latin1) as binary) using utf8)");
